...
export async function GET() {
  try {
    const fetchsnacks = await prisma.snacks.findMany();
    return NextResponse.json(fetchsnacks, { status: 200 });
  } catch (error) {
    console.log(error);
...
export async function POST(req: Request) {
  try {
    ...
    const Addsnacks = await prisma.snacks.create({
      data: {
        userEmail,
        FoodItems,
        CalorieIntake,
...
export async function PATCH(req: Request) {
  try {
    ...
    const updatesnacks = await prisma.snacks.update({
      where: {
        id,
      },
      data: {
...
export async function DELETE(req: Request) {
  try {
    ...
    const deleteFood = await prisma.snacks.delete({
      where: {
        id,
      },
    });
